...
const AUTOBOT_API_BASE_URL =
  process.env.AUTOBOT_API_BASE_URL?.replace(/\/$/, "") || "";
...
const parseProxyTarget = (pathname: string): string => {
  ...
  const base = AUTOBOT_API_BASE_URL.endsWith("/")
    ...
    res.status(500).json({
      error:
        error instanceof Error
          ? error.message
          : "Failed to proxy API request to autobot service",
    });
  }
});
...
app.get("/dashboard", (_req, res) => {
  res.sendFile(path.join(webPublicPath, "dashboard.html"));
});
